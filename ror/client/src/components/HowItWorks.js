...
function HowItWorks() {
  ...
  return (
    <Container>
      ...
      <h3>Overview</h3>
      <p>Mini Maestro uses the techniques of Counterpoint to guide the user by limiting options available to them. Instead of being able to place any note on the staff paper, the notes available to the user are limited based on the harmonic and melodic context of the current state of the piece. For example, if an F was just placed on the sheet, B would not be available in the list of options to choose from for the following note because that would result in a leap of a tritone, which is not allowed by the contrapuntal (meaning of or in Counterpoint) "rulebook". This program features JavaScript modules written to handle the logic of following these rules and providing viable options to the user, based on those rules. The user then can focus on selecting any note from the available list of options and will most likely achieve success in creating a piece that is audibly pleasing, regardless of the user's musical background (or lack thereof).</p>
      ...
      <h4>Harmonic</h4>
      <ul>
        <li>Voicing: Voices may not pass each other. For example the alto cannot rise higher than the soprano on any given beat.</li>
      ...
      <ul>
        ...
        <li>Tritone runs: A voice may not melodically outline a tritone (an interval exactly 6 half-steps away). In other words, if a voice has F, G, A, B, the following note cannot be A, because moving from F to B and back down would outline a tritone.</li>
